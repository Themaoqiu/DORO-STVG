...
# CompSTVG SFT dataset (DORO-STVG benchmark, mose+perception).
# Prompt + assistant target schema is kept identical to
# DORO-STVG/eval/prompts.py so that an SFT'd checkpoint can be evaluated
# without changing the eval pipeline.
_COMPSTVG_SYSTEM_PROMPT = (
    "You are an expert in spatiotemporal video grounding tasked with "
    "precisely locating objects/subjects in videos. You should output the "
    "space-time tube for each object the user intends to find."
)

_COMPSTVG_USER_PROMPT = (
    "Where does {query} occur in the video? Please find the location of the "
    "corresponding subject/object in this video. Give the corresponding "
    "bounding boxes for the object(s) in each corresponding frame.\n\n\n"
    "Guidelines:\n\n"
    "- Use normalized box coordinates in [0, 1].\n\n"
    "- Do not output explanations.\n\n"
    "- You must follow the exact output format below, and only output the "
    "format without any additional text or explanation.\n\n"
    "- Output a strict JSON object whose keys are target descriptions.\n\n"
    "- Each target description maps to a JSON object whose keys are frame "
    "indices as strings and whose values are normalized boxes "
    "[x1, y1, x2, y2].\n\n"
    "- Put frame indices first. Do not output timestamps.\n\n"
    "- Example for one target:\n\n"
    '  {{"chair": {{"12": [0.10, 0.20, 0.30, 0.40], '
    '"13": [0.11, 0.20, 0.31, 0.40]}}}}\n\n'
    "- Example for multiple targets:\n\n"
    '  {{"chair": {{"12": [0.10, 0.20, 0.30, 0.40]}}, '
    '"table": {{"12": [0.40, 0.50, 0.70, 0.90]}}}}\n\n'
)


def _compstvg_build_target_json(item):
    """Build the assistant JSON target string from one CompSTVG record.

    Each ``target_members`` entry is keyed by its per-target description
    (falling back to ``query`` for single-target items) and contains the
    per-frame normalized boxes.
    """
    width = float(item.get("video_width") or 0.0)
    height = float(item.get("video_height") or 0.0)
    if width <= 0 or height <= 0:
        return None

    members = item.get("target_members") or []
    per_target_queries = item.get("per_target_queries") or {}
    fallback_desc = str(item.get("query") or "")
    ...
    for member in members:
        ...
        desc = per_target_queries.get(f"target {target_index}") or fallback_desc
        desc = str(desc).strip()
        if not desc:
            continue
        ...
        if frame_map:
            # If two targets share the same description (rare), suffix to keep both.
            if desc in payload:
                desc = f"{desc} (target {target_index})"
            payload[desc] = frame_map
...
def make_compstvg_sft_dataset(
    path_or_dataset: str,
    video_dir: str,
    split: str = "train",
    **kwargs,
):
    """Load a CompSTVG queries jsonl as a VLM SFT dataset.

    Args:
        path_or_dataset: Path to a `.jsonl` produced by
            ``scripts/split_compstvg_sft_rl.py`` (or any compatible file).
        video_dir: Directory that the records' relative ``video_path`` are
            resolved against.
        split: Unused; accepted for parity with other ``make_*`` helpers.
        **kwargs: Ignored.

    Returns:
        list[dict]: Each entry has a ``conversation`` field formatted in the
        NeMo VLM convention (system + user with video + assistant JSON).
    """
    del split, kwargs  # unused
    ...
    with open(path_or_dataset, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if not os.path.isabs(video_abs):
                # Try direct join, then per-source subdirs (mose / perception).
                candidates = [os.path.join(video_root, video_rel)]
                source = item.get("source", "")
                if "mose" in source.lower():
                    candidates.append(os.path.join(video_root, "MOSE_1_fps", video_rel))
                if "perception" in source.lower() or "preception" in source.lower():
                    candidates.append(
                        os.path.join(video_root, "Perception_Test_Challenge_1_fps", video_rel)
                    )
                video_abs = next((p for p in candidates if os.path.exists(p)), None)
                if video_abs is None:
                    skipped_no_video += 1
                    continue
            elif not os.path.exists(video_abs):
                skipped_no_video += 1
                continue

            target_json = _compstvg_build_target_json(item)
            if target_json is None:
                skipped_no_target += 1
                continue

            query = str(item.get("query") or "").strip()
            user_text = _COMPSTVG_USER_PROMPT.format(query=query)
                    ...
                    "conversation": [
                        {
                            "role": "system",
                            "content": [
                                {"type": "text", "text": _COMPSTVG_SYSTEM_PROMPT}
                            ],
                        },
                        {
                            "role": "user",
    ...
    if skipped_no_video or skipped_no_target:
        logger.warning(
            "CompSTVG: loaded %d, skipped %d (missing video) %d (no target)",
            len(examples),
            skipped_no_video,
            skipped_no_target,
        )
    else:
        logger.info("CompSTVG: loaded %d samples", len(examples))
    return examples
