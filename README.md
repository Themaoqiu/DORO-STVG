...
- `results.jsonl`: per-sample predictions, parsed outputs, GT, and metrics
...
- [`graph_generator/main.py`](graph_generator/main.py): main scene graph generation entry
- [`graph_generator/modules/query_generator_cpsat.py`](graph_generator/modules/query_generator_cpsat.py): generate queries from scene graphs
- [`graph_generator/utils/format_train.py`](graph_generator/utils/format_train.py): convert query outputs into training format
- [`graph_generator/scripts/run_generator.sh`](graph_generator/scripts/run_generator.sh): current command collection used in practice
...
`eval/utils/cgstvg_infer_helper.py` and `eval/utils/tastvg_infer_helper.py`
implement this contract independently. On the server, point `CGSTVG_INFER_PY`
or `TASTVG_INFER_PY` to these helpers, and point `CGSTVG_PYTHON` or
`TASTVG_PYTHON` to the Python executable in the corresponding official model
checkout.
...
`graph_generator` depends on both model checkpoints and API-related environment variables. The scripts can load `graph_generator/.env` automatically when it is present.
...
For those details, refer to [`graph_generator/README.md`](graph_generator/README.md).
...
`eval/utils/cgstvg_infer_helper.py` probes the real video frame count with `ffprobe` and creates full-video dummy boxes so official prediction frame ids map back to DORO-STVG frame ids. `eval/utils/tastvg_infer_helper.py` uses a separate TA-STVG bridge, prepares the TA-STVG cache when requested, and writes the minimal action/attribute label stubs required by the official TA-STVG test script.
