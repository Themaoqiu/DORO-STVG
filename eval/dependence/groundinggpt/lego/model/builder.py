...
def load_pretrained_model(model_path, load_8bit=False, load_4bit=False, device_map="auto"):
    vision_tower = os.getenv(
        "GROUNDINGGPT_CLIP_VISION_TOWER",
        "/mnt/sdc/xingjianwang/yibowang/GroundingGPT/ckpt/clip-vit-large-patch14-336",
    )
    config = AutoConfig.from_pretrained(model_path)
    config.mm_vision_tower = vision_tower
    model = LEGOLlamaForCausalLM.from_pretrained(model_path, config=config, torch_dtype=torch.bfloat16).to(CONFIG.device)
    ...
    image_processor = CLIPImageProcessor.from_pretrained(vision_tower, torch_dtype=torch.bfloat16)
    image_tower = CLIPVisionModel.from_pretrained(vision_tower, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True).to(CONFIG.device)
