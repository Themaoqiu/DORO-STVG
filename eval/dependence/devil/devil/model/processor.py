...
from ..mm_utils import load_video_new, load_images
...
class DeViLProcessorKwargs(ProcessingKwargs, total=False):
    _defaults = {
        "text_kwargs": {
            "padding": False,
        },
...
class DeViLProcessor(ProcessorMixin):
    r"""
    ...
    def load_video(self, *args, **kwargs):
        return load_video_new(*args, **kwargs)
        ...
        **kwargs,
        ...
        for message_idx, message in enumerate(text):
            ...
            for chunk_idx in range(len(prompt_chunks) - 1):
                ...
                # if chunk_idx == 0:
                #     prompt.append(DEFAULT_GLOBAL_QUERY_TOKEN)
                thw = grid_sizes[image_idx]
                prompt.append(DEFAULT_IMAGE_TOKEN * thw.prod().long())
                # prompt.append(DEFAULT_LOCAL_QUERY_TOKEN)
                image_idx += 1
    ...
    def __call__(
        ...
        merge_size: Optional[int] = 1,
        return_labels: bool = False,
        **kwargs: Unpack[DeViLProcessorKwargs],
    ) -> BatchFeature:
        """
