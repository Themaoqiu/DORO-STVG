    ...
    def encode_images(self, images, split_sizes=None, modalities=None, 
        ...
        for modality, image in zip(modalities, images):
            ...
            if self.model.has_init_specific_embeddings:
                ...
                # LAPE
                def _match_frames(x, t):
                    if x.shape[0] == t:
                        return x
                    if x.shape[0] > t:
                        return x[:t]
                    pad = x[-1:].expand(t - x.shape[0], *x.shape[1:])
                    return torch.cat([x, pad], dim=0)
                t = image_feature.shape[0]
                resize_temporal_information_injection = _match_frames(resize_temporal_information_injection, t)
                resize_spatial_height_information_injection = _match_frames(resize_spatial_height_information_injection, t)
                resize_spatial_width_information_injection = _match_frames(resize_spatial_width_information_injection, t)
        ...
        if (input_ids.shape[1] != 1 or self.training):
            ...
            for cur_video_idx,(cur_input_ids, cur_input_embeds, modality) in enumerate(zip(input_ids, inputs_embeds, modalities)):
                ...
                if modality == 'image':
                    ...
                    if self.vision_config.vid_start_token is None:
                        video_start_tokens = torch.empty(0, dtype=torch.long, device=cur_input_ids.device)
                    else:
                    ...
                    if getattr(self.vision_config, "slow_token", False):
                        slow_vid_start_token = getattr(self.vision_config, "slow_vid_start_token", None)
                        slow_vid_end_token = getattr(self.vision_config, "slow_vid_end_token", None)
                        if slow_vid_start_token is None or slow_vid_end_token is None:
                            slow_video_start_tokens = torch.empty(0, dtype=torch.long, device=cur_input_ids.device)
                        else:
                            slow_video_start_tokens = torch.where(cur_input_ids == slow_vid_start_token)[0]
                        ...
                        for video_start_token_pos in slow_video_start_tokens:
                            ...
                            cur_video_features = cur_video_features.flatten(0, 1)
                            num_patches = cur_video_features.shape[0]
                            if cur_input_ids[video_start_token_pos + num_patches - 1] != slow_vid_end_token:
                                raise ValueError("The video end token should follow the video start token.")
                            if orig_embeds_params is not None:
                                cur_new_input_embeds = torch.cat((
                                    cur_input_embeds[:video_start_token_pos],
