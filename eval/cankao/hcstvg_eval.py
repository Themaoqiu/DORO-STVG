...
class HCSTVGiouEvaluator:
    ...
    def evaluate(self, predictions: List[Dict], video_predictions: List[Dict], pred_kf: List[Dict]):
        vid_metrics = {}
    ...
    def summarize(self):
        if is_main_process():
            ...
            self.results, vid2names, vid2sents = self.evaluator.evaluate(
                self.predictions, self.video_predictions, self.kf_pred
            )
            
            metrics = {"gt_viou": 0}
            metrics.update({"tiou": 0, "viou": 0})
            metrics.update({"kf_p": 0, "kf_r": 0})
            ...
            for x in self.results.values():  # sum results
                ...
                metrics["kf_p"] += x['kf_pr'][0]
                metrics["kf_r"] += x['kf_pr'][1]
                counter += 1
            ...
            fail_threshold = max(self.iou_thresholds) if self.iou_thresholds else 0.0
            failed_queries = []
            for vid, metrics in self.results.items():
                if metrics["viou"] < fail_threshold:
                    failed_queries.append({
                        "item_id": vid,
                        "video_name": vid2names.get(vid),
                        "query": vid2sents.get(vid),
                        "viou": metrics["viou"],
                        "tiou": metrics["tiou"],
                        "gt_viou": metrics["gt_viou"],
                        "pred_sted": metrics["pred_sted"],
                        "gt_sted": metrics["gt_sted"],
                        "threshold": fail_threshold,
                    })

            fail_filename = f"failed_queries_{self.evaluator.subset}.json"
            if self.save_dir:
                os.makedirs(self.save_dir, exist_ok=True)
                fail_path = os.path.join(self.save_dir, fail_filename)
            else:
                fail_path = fail_filename
            save_json(fail_path, failed_queries)
            self.logger.info(f"Saved {len(failed_queries)} failed queries to {fail_path}")

            out["failed_queries"] = failed_queries
            out["failed_queries_path"] = fail_path
