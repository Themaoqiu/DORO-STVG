...
def _build_tubedetr_args(repo_dir: Path, checkpoint: str, dataset_config: str, combine_datasets: List[str], combine_datasets_val: List[str], resolution: int, fps: int, device: str, output_dir: str):
    sys.path.insert(0, str(repo_dir))
    from main import get_args_parser
    parser = argparse.ArgumentParser("TubeDETR helper", parents=[get_args_parser()])
    cli = [
...
def main():
    ...
    parser.add_argument("--tubedetr-dir", required=True)
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--dataset-config", default="config/vidstg.json")
    ...
    repo_dir = Path(args.tubedetr_dir).expanduser().resolve()
    os.chdir(repo_dir)
    tube_args = _build_tubedetr_args(repo_dir, args.checkpoint, args.dataset_config, args.combine_datasets, args.combine_datasets_val, args.resolution, args.fps, args.device, args.tmp_output_dir or str(repo_dir / "_helper_outputs"))
