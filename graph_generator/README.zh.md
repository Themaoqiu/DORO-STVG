...
```bash
cd DORO-STVG/graph_generator/dependence/GroundedSAM2/checkpoints
bash download.sh
```

如果只需要当前脚本使用的 SAM2.1 large 权重，也可以直接下载：
...
- `scene_graphs.jsonl` 中的 `video_path` 现在只保存视频文件名，不再写入绝对路径。
- full pipeline 中自动生成的 `output/sam2_masks/*_sam2_masks_indexed.json` 会在属性阶段完成后自动删除，作为中间文件处理。
...
| `--groundedsam2_mask_output_dir` | 临时 SAM2 索引掩码输出目录。 |
| `--sam3_redetection_interval` | 跟踪器重检测间隔，也会影响关键帧间隔。 |
...
| `--relation_crop_output_dir` | 关系阶段裁剪图和可选中间文件输出目录。 |
...
- `output/scene_graphs.jsonl`：生成的场景图
- `output/relation_crops/`：关系模块裁剪图（可选）
