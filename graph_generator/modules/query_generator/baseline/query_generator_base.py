...
PROMPT_TEMPLATE = (
    ...
    "9) If disambiguation guidance is provided, you MUST include exactly one extra clue from the candidate pool; prefer environment interaction/context relation over appearance when available.\n"
    "10) Output exactly one query string in strict JSON only: {{\"query\": \"...\"}}.\n\n"
    "Clue summary:\n"
    "{clue_text}"
)
...
def _prompt_from_spec(spec: Dict[str, Any]) -> str:
    ...
    if spec.get("action_sequences"):
        ...
        for item in spec.get("action_sequences", []):
            if isinstance(item, dict):
                seq = [_norm(x) for x in (item.get("sequence") or []) if _norm(x)]
                if not seq:
                    seq = [_norm(item.get("first")), _norm(item.get("second"))]
                    seq = [x for x in seq if x]
                text = " before ".join(seq) if len(seq) >= 2 else ""
    ...
    req = spec.get("required_disambiguation") or {}
    req_type = _norm(req.get("type", "")).lower()
    req_value = _norm(req.get("value", ""))
    if req_type and req_value:
        lines.append(f"- disambiguation required: add exactly one {req_type} clue -> {req_value}")
    cand = spec.get("disambiguation_candidates") or {}
    cand_ctx = _uniq(cand.get("context_relations", []))
    cand_attr = _uniq(cand.get("attributes", []))
    if cand_ctx or cand_attr:
        lines.append("- disambiguation candidate pool:")
        if cand_ctx:
            lines.append("  context relations: " + "; ".join(cand_ctx))
        if cand_attr:
            lines.append("  appearance attributes: " + "; ".join(cand_attr))
    scene = spec.get("scene_object_clues") or []
    if scene:
        lines.append("- scene object clues (all nodes):")
        for obj in scene:
            if not isinstance(obj, dict):
                continue
            cls = _norm(obj.get("class", "object")) or "object"
            attrs = ", ".join(_uniq(obj.get("attributes", []))) or "-"
            ctx = ", ".join(_uniq(obj.get("context_relations", []))) or "-"
            lines.append(f"  {cls}: attrs[{attrs}] ctx[{ctx}]")
    return "\n".join(lines)
...
def _clue_signature(spec: Dict[str, Any]) -> str:
    ...
    signature = {
        ...
        "action_sequences": _sorted_signature_items([
            {
                "sequence": [
                    _norm(x).lower()
                    for x in ((item.get("sequence") or [_norm(item.get("first", "")), _norm(item.get("second", ""))]))
                    if _norm(x)
                ],
        ...
        "required_disambiguation": {
            "type": _norm((spec.get("required_disambiguation") or {}).get("type", "")).lower(),
            "value": _norm((spec.get("required_disambiguation") or {}).get("value", "")).lower(),
        },
...
def _ordered_action_sequences(action_map: Dict[str, Set[int]], max_len: int = 4) -> List[List[str]]:
    labels = sorted([label for label, frames in action_map.items() if frames])
    starts = {label: min(action_map[label]) for label in labels}
    ends = {label: max(action_map[label]) for label in labels}
    can_follow = {
        a: [b for b in labels if a != b and ends[a] < starts[b]]
        for a in labels
    }
    out: List[List[str]] = []
    ...
    def dfs(path: List[str]) -> None:
        ...
        if len(path) >= max_len:
            return
        for nxt in can_follow.get(path[-1], []):
            if nxt in path:
                continue
            dfs(path + [nxt])
...
def _scene_object_clues(objects: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def _best_disambiguation_for_target(
    ...
    target_ids = [node_id for node_id in target_group if node_id in objects]
    original_attr = {}
    original_ctx = {}
    for node_id in target_ids:
        if not (object_frames.get(node_id, set()) & seg_frames):
            continue
    ...
    if not active_distractors:
        pool = {
            "context_relations": list(original_ctx.values())[:8],
            "attributes": list(original_attr.values())[:8],
        }
        req = None
        if pool["context_relations"]:
            req = {"type": "context_relation", "value": pool["context_relations"][0]}
        elif pool["attributes"]:
            req = {"type": "attribute", "value": pool["attributes"][0]}
        return req, pool

    distractor_attr_sets = []
    distractor_ctx_sets = []
    ...
    for low, orig in original_ctx.items():
        miss = sum(1 for s in distractor_ctx_sets if low not in s)
        scored_ctx.append((miss, orig))
    scored_ctx.sort(key=lambda x: (-x[0], x[1].lower()))

    scored_attr: List[Tuple[int, str]] = []
    for low, orig in original_attr.items():
        miss = sum(1 for s in distractor_attr_sets if low not in s)
        scored_attr.append((miss, orig))
    scored_attr.sort(key=lambda x: (-x[0], x[1].lower()))

    pool = {
        "context_relations": [x[1] for x in scored_ctx[:8]],
        "attributes": [x[1] for x in scored_attr[:8]],
    }
    req = None
    if pool["context_relations"]:
        req = {"type": "context_relation", "value": pool["context_relations"][0]}
...
class MinimalSTVGQueryGenerator:
    def __init__(
        ...
        w1: float = 0.6,
        w2: float = 0.4,
        max_action_sequence_len: int = 7,
    ) -> None:
        self.max_combo_size = max_combo_size
        self.max_action_sequence_len = max(2, int(max_action_sequence_len))
        total = float(w1) + float(w2)
        if total <= 0:
            self.w1, self.w2 = 0.6, 0.4
        else:
            self.w1, self.w2 = float(w1) / total, float(w2) / total
    ...
    def build_query_plan(
        ...
        for node_id in target_group:
            sequences = _ordered_action_sequences(action_frames[node_id], max_len=self.max_action_sequence_len)
            for seq in sequences:
                seq_frames = set()
                ok_seq = True
                for label in seq:
                    frames = action_frames[node_id].get(label, set())
                    if len(frames) < 2:
                        ok_seq = False
                        break
                    seq_frames |= frames
                ...
                for d in distractor_ids:
                    valid = True
                    d_frames_union = set()
                    prev_frames: Optional[Set[int]] = None
                    for label in seq:
                        cur = action_frames[d].get(label, set())
                        if len(cur) < 2:
                            valid = False
                            break
                        if prev_frames is not None and max(prev_frames) >= min(cur):
                            valid = False
                            break
                        d_frames_union |= cur
                        prev_frames = cur
                    d_support[d] = d_frames_union if valid else set()
                add(
                    "action_sequence",
                    {"action_sequences": [{"sequence": seq, "order": "before", "cross_shot": False}]},
                    seq_frames,
                    d_support,
                )
                ...
                for first, f1 in action_frames[ref_id].items():
                    ...
                    for second, f2 in action_frames[second_id].items():
                        ...
                        add(
                            "cross_shot",
                            {
                                "action_sequences": [{"sequence": [first, second], "order": "before", "cross_shot": True}],
        ...
        for seg, must_use_local in segment_pool:
            ...
            for k in range(1, min(combo_limit, len(elements)) + 1):
                ...
                for combo in combinations(elements, k):
                    ...
                    support_segments = set(_frames_to_segments(support))
                    # Enforce that the selected time segment must come from clue-supported segments directly.
                    if seg not in support_segments:
                        continue
                    # Temporal uniqueness: if not using full track, this clue combo should map to exactly one segment.
                    if seg != (track_start, track_end) and len(support_segments) != 1:
                        continue
                    if must_use_local and not any(
                        elem["type"] in {"action", "action_target", "relation", "relation_described", "action_sequence", "cross_shot"}
                        and seg in set(_frames_to_segments(set(elem["support"])))
                        for elem in combo
                    ):
                        continue
                    ok = True
        ...
        for item in candidates:
            ...
            spec = {
                ...
                "required_disambiguation": {},
                "disambiguation_candidates": {"context_relations": [], "attributes": []},
                "scene_object_clues": _scene_object_clues(objects),
            }
            for elem in combo:
                clue = elem["clue_data"]
            ...
            total_action_relation = len(spec["actions"]) + len(spec["action_targets"]) + len(spec["relations"])
            if total_action_relation == 1 and not spec["action_sequences"] and not spec["attributes"] and not spec["context_relations"]:
                req, pool = _best_disambiguation_for_target(
                    target_group=default_same_entity_nodes,
                    distractor_ids=distractor_ids,
                    objects=objects,
                    object_frames=object_frames,
                    seg_frames=set(range(seg_start, seg_end + 1)),
                )
                spec["required_disambiguation"] = req or {}
                spec["disambiguation_candidates"] = pool
                if req:
                    if req.get("type") == "context_relation":
                        if req["value"] not in spec["context_relations"]:
                            spec["context_relations"].append(req["value"])
                    elif req.get("type") == "attribute":
                        if req["value"] not in spec["attributes"]:
                            spec["attributes"].append(req["value"])
            ...
            max_sequence_len = 0
            for seq_item in spec["action_sequences"]:
                if not isinstance(seq_item, dict):
                    continue
                seq_vals = [_norm(x) for x in (seq_item.get("sequence") or []) if _norm(x)]
                if not seq_vals:
                    seq_vals = [_norm(seq_item.get("first")), _norm(seq_item.get("second"))]
                    seq_vals = [x for x in seq_vals if x]
                max_sequence_len = max(max_sequence_len, len(seq_vals))

            if has_cross_shot:
                t_level = 4
            elif has_sequence:
                t_level = min(4, max(2, 1 + max_sequence_len))
            ...
            active_ids = [obj_id for obj_id, frames in object_frames.items() if frames & seg_frames]
            same_class_active = sum(1 for d in distractor_ids if object_frames.get(d, set()) & seg_frames)
            active_count_score = min(max(len(active_ids) - 1, 0) / 8.0, 1.0)
            same_class_score = min(same_class_active / 3.0, 1.0)
            crowd_score = 0.4 * active_count_score + 0.6 * same_class_score

            pairwise_tiou: List[float] = []
            if len(active_ids) >= 2:
                for left, right in combinations(active_ids, 2):
                    lf = set(object_frames.get(left, set())) & seg_frames
                    rf = set(object_frames.get(right, set())) & seg_frames
                    pairwise_tiou.append(_temporal_iou(lf, rf))
            avg_pairwise_tiou = (sum(pairwise_tiou) / len(pairwise_tiou)) if pairwise_tiou else 1.0

            d_s = self.w1 * crowd_score + self.w2 * (1.0 - avg_pairwise_tiou)
            d_s = min(max(d_s, 0.0), 1.0)
            s_level = d_s
            d = 0.5 * d_t + 0.5 * d_s
    ...
    def build_graph_plans(
        ...
        _ = total_frames, d_star_list, balance_types, difficulty_bins, max_queries_per_target
        ...
        if balance_difficulty and plans:
            bucket_order = ["easy", "medium", "hard", "very_hard"]
            bucketed = {bucket: [] for bucket in bucket_order}
            tail = []
            for item in plans:
                bucket = str(item.get("difficulty_bucket", "medium"))
            ...
            while True:
                ...
                for bucket in bucket_order:
                    if bucketed[bucket]:
                        balanced.append(bucketed[bucket].pop(0))
                        added = True
                if not added:
                    break
        ...
        for item in query_spec.get("action_sequences", []):
            if isinstance(item, dict):
                ...
                if not seq:
                    ...
                    seq = [x for x in seq if x]
                if len(seq) >= 2:
                    chunks.append("that " + " before ".join(seq))
    ...
    def process_graph(
        ...
        for record, plan in zip(records, kept_plans):
            key = (
