...
def build_image_message(user_prompt: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    Note:
        For compatible-mode Qwen APIs, passing a local path or URL under the "image"
        field is sufficient. This helper normalizes common keys so image prompts
        are aligned with the multimodal API wrapper.
    """
    processed_content = []
